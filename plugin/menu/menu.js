...
var RevealMenu = window.RevealMenu || (function(){
	...
	//nie dziala - zwiastun	 - 3 wersy kodu
		...
		if (initialise) {
			...
			function init() {
				if (!initialised) {
					...
					addToolbarButton('Slajdy', 'Slides', 'fa-images', 'fas', openPanel, true);
					...
					if (themes) {
						addToolbarButton('Tematy', 'Themes', 'fa-adjust', 'fas', openPanel);
					}
					if (transitions) {
						addToolbarButton('Przejścia', 'Transitions', 'fa-sticky-note', 'fas', openPanel);
					}
					...
					button.appendChild(create('span', {'class': 'slide-menu-toolbar-label'}, 'Zamknij'));
